...
public class ModulesHelper { 
		...
		if (userName != null) {
			...
			if (user != null) {
				if (user.isForceChangePassword()) {   
					...
					boolean isMobile = Browsers.isMobile(request);
					for (Module module: user.getNotHiddenModules()) {
						if (isMobile) {
							if (module.isMobile()) all.add(module); 
						}
						else {
							if (module.isDesktop()) all.add(module);
						}
					}
				}
			}
	...
	public static boolean showsSearchModules(HttpServletRequest request) {
		...
		if (showsSearchModules == null) {
			...
			Collection<Folder> subfolders = folders.getSubfolders(request); 
			if (subfolders.size() == 1 && subfolders.iterator().next().getName().equals("Admin")) {
				showsSearchModules = false;
			}
			else showsSearchModules = !folders.getSubfolders(request).isEmpty() || modules.getAll(request).size() > 30; 
