...
public class InitSignInAction extends ViewBaseAction {  
	...
	public void execute() throws Exception {
		...
		if (NaviOXPreferences.getInstance().isShowOrganizationOnSignIn() &&
			...
			if (NaviOXPreferences.getInstance().isShowListOfOrganizationsOnSignIn()) {
				getView().setHidden("organization", organization != null || Organization.count() == 0);
				getView().setHidden("organizationName", true);
			}
			else {
				getView().setHidden("organizationName", organization != null || Organization.count() == 0);
