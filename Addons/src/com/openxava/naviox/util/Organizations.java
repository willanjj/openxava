...
public class Organizations {
	...
	public static boolean init(ServletRequest request, String organization) {
		...
		if (!Is.equal(organization, currentOrganization)) {
			if (Configuration.getInstance().isSharedUsersBetweenOrganizations()) {
				...
				User organizationUser = user == null?null:User.find((String) user);
				if (organizationUser != null && !organizationUser.isActive()) {
					XPersistence.commit();
					XHibernate.commit();
					return false;
				}
	...
	/** @since 6.2 */
	public static Organization create(String name, boolean active) {  
		return create(name, active, null, null, false); 
	}
	
	/** @since 6.4.2 */
	public static Organization create(String name, boolean active, String theme) { 
		return create(name, active, theme, null, false);
	}

	/** @since 5.6 */
	public static Organization create(String name, String adminUser) { 
		return create(name, true, null, adminUser, false); 
	}
	
	/** @since 6.0 */
	public static Organization createWithBlankDB(String name) {  
		return create(name, true, null, null, true); 
	}
	 	
	private static Organization create(String name, boolean active, String theme, String adminUser, boolean blankDB) { 
